...
static int mbox;
int count, space;
char Mspace[CHAR_MSG_SIZE];
msg_t *m = (msg_t *)Mspace;
...
int getchar(int *c)
{
	// We have to enter critical so we cant be interrupted in while getting characters
	enter_critical();
	mbox_recv(mbox, m);
	leave_critical();
	*c = m->body[0];
...
{
	mbox_stat(QUEUE, &count, &space);
	if(space > CHAR_MSG_SIZE){
		m->size = 1;
