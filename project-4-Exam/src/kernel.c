...
#pragma GCC diagnostic ignored "-Wcast-function-type"
	...
	init_syscall(SYSCALL_KILLPROC, (syscall_t)killproc);
	init_syscall(SYSCALL_PSPROC, (syscall_t)psproc);
	init_syscall(SYSCALL_SUSPROC, (syscall_t)susproc);
	init_syscall(SYSCALL_RESPROC, (syscall_t)resproc);
...
{
	asm volatile(
		"movl %%eax, %%cr3 " ::"a"(current_running->page_directory));
}
...
static void init_idt(void)
{
	...
	for (i = 0; i < NUM_EXCEPTIONS; i++) {
		create_gate(&(idt[i]), /* IDT entry */
			    ...
			    (uint32_t)exception_handler[i], KERNEL_CS,
	 ...
	 */
	create_gate(&(idt[IRQ_START + 7]), (uint32_t)fake_irq7_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);

	/* Create gate for the timer interrupt */
	create_gate(&(idt[IRQ_START]), (uint32_t)irq0_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);

	/* Create gate for the keyboard interrupt */
	create_gate(&(idt[IRQ_START + 1]), (uint32_t)irq1_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);

	/* Create gates for the PCI interrupts */
	create_gate(&(idt[IRQ_START + 5]), (uint32_t)pci5_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);
	create_gate(&(idt[IRQ_START + 9]), (uint32_t)pci9_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);
	create_gate(&(idt[IRQ_START + 10]), (uint32_t)pci10_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);
	create_gate(&(idt[IRQ_START + 11]), (uint32_t)pci11_entry, KERNEL_CS,
		    INTERRUPT_GATE, 0);

	/* Create gate for system calls */
	create_gate(&(idt[IDT_SYSCALL_POS]), (uint32_t)syscall_entry, KERNEL_CS,
		    INTERRUPT_GATE, 3);
...
void init_gdt(void)
{
	...
	create_segment(gdt + TSS_INDEX, (uint32_t)&tss, TSS_SIZE, TSS_SEGMENT,
		       0,	/* highest privilege level */
...
{
	static char *status[] = {"Running ", "Blocked ", "Sleeping", "Suspended",
				 "Exited  "};
	int i, j, base;
	pcb_t *p;
	...
	scrprintf(base - 5, 12,
		  "P R O C E S S    S T A T U S   after %d seconds", time);
	scrprintf(base - 3, 0, "%-5s%-10s%-10s%-10s%-10s%-10s%-10s%-10s", "Pid",
		  "Type", "Status", "Disable", "Preempt", "Yield", "Page",
		  "Kernel");
	...
	do {
		scrprintf(base + i, 0,
			  "%-5d%-10s%-10s%-10d%-10d%-10d%-10d%-10x", p->pid,
			  p->is_thread ? "Thread" : "Process",
			  status[p->status], p->disable_count, p->preempt_count,
			  p->yield_count, 0, p->kernel_stack);
		p = p->next;
		i++;
...
int readdir(char *buf)
{
	...
	// Read the the whole bootblock data
	scsi_read(0, 1, tempBuf);

	// Copy bytes 2-4 from tempbuf and into OS_size
	bcopy(tempBuf + 2, &OS_size, 2);

	// Read from where the kernel ends + 1 to access process directory
	scsi_read(OS_size + 1, 1, tempBuf);

	// Copy process directory data into buf
	bcopy(tempBuf, buf, SECTOR_SIZE);
	return 0;
}
...
{
	// (512 * sectors) + 8192
	unsigned int trueSize = (SECTOR_SIZE * size) + STACK_SIZE;

	// Allocate memory for the process and its stack
	char *process = alloc_memory(trueSize);

	// Read process data
	scsi_read(location, size, process);

	// Create process with the data read
	create_process(process, trueSize);
	return 0;
}

int killproc(int pid)
{
	pcb_t *curr_KILL = current_running;
	int start_pid = curr_KILL->pid;
	int amount_killed = 0;
	if (same_string(pid, "all")) {
		// While we're not on the pid we want
		while (curr_KILL->pid != pid) {
			curr_KILL = curr_KILL->next;
			// Check if we looped around
			if (curr_KILL->pid == start_pid) {
				// If something was killed, return the amount
				if (amount_killed > 0) {
					return amount_killed;
				} else {
					return 0;
				}
			// Kill if PID is above 5
			} else if (curr_KILL->pid > 5) {
				curr_KILL->status = EXITED;
				amount_killed++;
			}
		}
	// If the user only wants one process killed
	} else {
		// While we're not on the pid we want
		while (curr_KILL->pid != pid) {
			curr_KILL = curr_KILL->next;
			// Return nothing if we looped around
			if (curr_KILL->pid == start_pid) {
				return 0;
			}
		}
...
{
	pcb_t *temp = current_running->next;
	int count = 0;
	int first_pid = temp->pid;
	int searching = TRUE;

	clear_screen(0, 0, 25, 9);
	while (searching) {
		temp = temp->next;
		// Find a pid where it's lower than 6 and the next is bigger than 5
		// By doing this we find the thread before the first active process
		if (temp->pid < 6 && temp->next->pid > 5) {
			temp = temp->next;
			searching = FALSE;
		// Return if there are no pids above 5
		} else if (temp->pid == first_pid) {
			return count;
		}
	}

	// store the processes with PIDs greater than 5
	while (temp->pid > 5) {
		ps[count].pid = temp->pid;
		// Convert status from int to readable text
		switch (temp->status) {
		case RUNNING:
			bcopy("RUNNING", ps[count].status, 10);
			break;
		case BLOCKED:
			bcopy("BLOCKED", ps[count].status, 10);
			break;
		case SLEEPING:
			bcopy("SLEEPING", ps[count].status, 10);
			break;
		case SUSPENDED:
			bcopy("SUSPENDED", ps[count].status, 10);
			break;
		case EXITED:
			bcopy("EXITED", ps[count].status, 10);
			break;
		default:
			bcopy("ERROR", ps[count].status, 10);
			break;
		}
		count++;
		/*
		* Sometimes the shell is thrown into the queue, which exits us
		* out of the while loop above. We do not want this because
		* there are times where we want to store these pids
		* 6 - 7 - 8 - 9
		* and if the shell with PID 5 comes inbetween 7 and 8
		* it will exit the while loop and not store 8 and 9
		*/
		if (temp->next->pid < 6) {
			temp = temp->next->next;
		} else {
	...
	// Return count for loop and error check
	return count;
}

int susproc(pid){
	pcb_t *curr_SUS = current_running;
	int start_pid = curr_SUS->pid;

	while (curr_SUS->pid != pid) {
		curr_SUS = curr_SUS->next;
		if (curr_SUS->pid == start_pid) {
			return 0;
		}
	}
	if (curr_SUS->status == 3)
		{
			return -1;
		}
	curr_SUS->status = SUSPENDED;
	return pid;
}
...
int resproc(pid){
	...
	if (curr_SUS->status == 0)
		{
			return -1;
		}
	curr_SUS->status = RUNNING;
	return pid;
}