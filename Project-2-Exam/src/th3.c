...
enum
{
	...
	MCPI_Y = 20,     // Line where results are printed
	MCPI_X0 = 25,
	MCPI_X1 = 35,
	MCPI_X2 = 45,
	MCPI_X3 = 55,
...
void mcpi(int cnt, int py, int px) {
	...
	for (i = 0; i < cnt; i++) {
		x = my_rand(x + 0.43242 - y);
		y = my_rand(y + x + 0.83192478912);
		// x = my_rand(x);
		// y = my_rand(y);
		if (is_hit(x, y)) {
			hit++;
...
// FEUIO LMED BUNBLOCK FÅR 0
	exit();
	HALT("mcpi_thread0() should not return");
}
...
void mcpi_thread1(void) {
	...
	exit();
	HALT("mcpi_thread1() should not return");
}
...
void mcpi_thread2(void) {
	...
	exit();
	HALT("mcpi_thread2() should not return");
}
...
void mcpi_thread3(void) {
	...
	exit();
	HALT("mcpi_thread3() should not return");
}
